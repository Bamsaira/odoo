                ...
                if taxes:
                    ...
                    for tax in taxes:
                        if tax.get('@TasaOCuota'):
                            ...
                            tasa = str(0)
                        tax_exist = tax_obj.search([('impuesto','=',tax.get('@Impuesto')), ('type_tax_use','=','sale'), ('tipo_factor','=',tax.get('@TipoFactor')), ('amount', '=', tasa), ('company_id','=',self.env.company.id)],limit=1)
                        if not tax_exist:
                ...
                if taxes:
                    ...
                    for tax in taxes:
                        if tax.get('@TasaOCuota'):
                            ...
                            tasa = str(0)
                        tax_exist = tax_obj.search([('impuesto','=',tax.get('@Impuesto')), ('type_tax_use','=','purchase'), ('tipo_factor','=',tax.get('@TipoFactor')), ('amount', '=', tasa), ('company_id','=',self.env.company.id)],limit=1)
                        if not tax_exist:
                            raise Warning("La factura contiene impuestos que no han sido configurados. Por favor configure los impuestos primero")
                        tax_ids.append(tax_exist.id)
                        k = k+1
...
#                raise Warning("Factura ya existente con la referencia del vendedor %s"%(customer_reference))
    ...
    def get_or_create_product(self, default_code, product_name, clave_unidad, unit_price, clave_producto, sale_ok=True, purchase_ok=False):
        ...
        if not product_exist:
            ...
            product_vals = {'default_code':default_code, 'name':product_name, 'standard_price' : unit_price,'cat_unidad_medida' : um_descripcion.id, 'clave_producto' : clave_producto, }
